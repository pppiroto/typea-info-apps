...
<head>
  ...
  function resizeStacktracePanel() {
      ...
      stkPanel.style.height = (iheight * 0.5)  + "px";
      if (iWidth > W_MARGIN) {
   ...
   *
   */
  function dispDetail(linkTag) {
  	var dtlPanel = $("detailPanel");
  	var isBlock = (dtlPanel.style.display == "block");
  	if (isBlock) {
  		dtlPanel.style.display = "none";
  		linkTag.innerHTML = "詳細を表示";
  	} else {
  		dtlPanel.style.display = "block";
  		linkTag.innerHTML = "詳細を隠す";
  	}
  }
  //]]></script>
  <style type="text/css">
...
<body onload="javascript:resizeStacktracePanel();" onresize="javascript:resizeStacktracePanel();">
  ...
  <hr/>
  <div>
  	<a href="#" onclick="javascript:dispDetail(this);">詳細を表示</a>
  </div>
  <br/>
